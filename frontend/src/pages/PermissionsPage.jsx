...
export default function PermissionsPage() {
  ...
  useEffect(() => {
    ...
    if (auth.role === "admin") {
      ...
      if (focusParam) {
        window.setTimeout(() => {
          const targetElement = document.getElementById(focusParam);
          if (targetElement) {
            targetElement.scrollIntoView({ behavior: "smooth", block: "center" });
            targetElement.classList.add("ring-2", "ring-primary-400", "ring-offset-2");
            window.setTimeout(() => targetElement.classList.remove("ring-2", "ring-primary-400", "ring-offset-2"), 2500);
          }
        }, 150);
      }
    }
    ...
    try {
      ...
      await Promise.all(statusTabs.map(async (tab) => {
        ...
        if (tab.value === "approved" || tab.value === "rejected") {
          counts[tab.value] = announcementData.filter((ann) => !seenAnnouncements.includes(ann._id)).length + transactionData.length;
        } else {
  ...
  const refreshPermissionsPage = async () => {
    await loadStatusCounts();
    await loadApprovals(selectedStatus);
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {paginatedApprovals
                        ...
                        .map((ann) => (
                          <div id={`ann-${ann._id}`} key={ann._id} className="p-3 bg-slate-50 border border-slate-200 rounded-xl animate-fade-in">
                          {selectionMode && (
                  ...
                  <div id="payments">
                    ...
                    <div className="space-y-4">
                      {paginatedApprovals
                        ...
                        .map((tx) => (
                          <div id={`payment-${tx._id}`} key={tx._id} className="p-3 bg-slate-50 border border-slate-200 rounded-xl animate-fade-in">
                          {selectionMode && (
                            <div className="flex items-center gap-2 mb-2">
                              <input
                                type="checkbox"
