...
export default function Programs() {
  const { auth, schoolYear, semester, updateSemester } = useContext(AuthContext);
  ...
  const programTabs = ["HRT", "OAT", "OMT", "WADT"];
  const semesters = ["1ST SEM", "2ND SEM"];
  const programIcons = { HRT: "🏥", OAT: "⚙️", OMT: "🔧", WADT: "💻" };
  const programDesc = { HRT: "Hotel & Restaurant Technology", OAT: "Office Administration Technology", OMT: "Operations Management Technology", WADT: "Web and Application Development Technology" };
  ...
  const handleProgramClick = (program) => {
    navigate(`/program/${program}`, { state: { program, semester: selectedSemester } });
  };
  ...
  const handleProgramMenuToggle = (e, programId) => {
    e.stopPropagation();
    setOpenProgramMenuId((prev) => (prev === programId ? null : programId));
  };

  const handleEditProgramClick = (e, program) => {
    e.stopPropagation();
  ...
  const handleEditSectionClick = (e, programId, sectionName) => {
    e.stopPropagation();
    setSelectedProgramId(programId);
  ...
  const handleSectionUpdated = (updatedProgram) => {
    setPrograms(programs.map(p => p._id === updatedProgram._id ? updatedProgram : p));
  };
  ...
  const handleDeleteProgram = async (e, programId) => {
    ...
    const confirmMessage = `Are you sure you want to delete the program "${program?.name}"?\n\nThis will also delete all sections associated with this program. This action cannot be undone.`;
    ...
    <MainLayout user={{ name: "User" }} onMenuItemClick={(item) => { if (item === "home") navigate("/home"); }}>
      <div className="page-content">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 mb-6">
          <button onClick={() => navigate("/school-year")} className="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-100 transition-colors text-sm flex-shrink-0">←</button>
          <div className="flex-1">
            <h1 className="page-title mb-0 text-lg sm:text-2xl">📚 {schoolYear}</h1>
            <p className="page-subtitle text-[10px] sm:text-xs">Select Program to view students and payments</p>
          </div>
        </div>
        ...
        {selectedSemester && (
          <>
            <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
              <div className="flex-1">
                <h2 className="text-base sm:text-lg font-semibold">Browse by Program</h2>
              </div>
                <div className="flex gap-2">
                <button
                  onClick={() => setShowAddProgramModal(true)}
                  className="btn-primary text-xs sm:text-sm whitespace-nowrap w-full sm:w-auto"
                >
                  + Add Program
                </button>
              </div>
            ...
            <div className="mb-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <button
                  onClick={() => setShowAddProgramModal(true)}
                  className="card-hover flex items-center justify-center text-center p-4 sm:p-5 group hover:border-primary-200 border-2 border-slate-100 bg-white min-h-[120px] sm:min-h-auto"
                >
                  <span className="text-primary-700 font-semibold text-sm sm:text-base">+ Add Program</span>
                </button>
                {programTabs.map(program => (
                  ...
                  >
                    <span className="icon-theme icon-theme-lg">
                      {programIcons[program]}
                    </span>
                    <div className="flex-1">
                      <h3 className="font-display font-bold text-slate-900 mb-1">{program}</h3>
                      <p className="text-xs text-slate-500">{programDesc[program]}</p>
                    </div>
              ...
              {loadingPrograms ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {programs.map(program => (
                    <div
                      key={program._id}
                      className="border-2 rounded-xl overflow-hidden transition-colors border-slate-100 hover:border-primary-200"
                    >
                      <div
                          ...
                          <div className="flex items-start gap-2">
                            {auth?.role === "admin" && (
                              <div className="relative">
                                <button
                                  type="button"
                                  onClick={(e) => handleProgramMenuToggle(e, program._id)}
                                  className="text-slate-400 hover:text-slate-600 text-xl px-2 py-1 rounded-full hover:bg-slate-100"
                                  title="Program actions"
                                >
                                  ⋯
                                </button>
                                {openProgramMenuId === program._id && (
                                  <div className="absolute right-0 top-full mt-2 w-32 rounded-xl border bg-white shadow-lg z-10">
                                    <button
                                      onClick={(e) => handleEditProgramClick(e, program)}
                            ...
                            program.sections.map(section => (
                              <div key={section} className="flex items-center justify-between bg-white p-2 rounded border border-slate-100">
                                ...
                                <div className="flex items-center gap-2">
                                  <button
                                    onClick={(e) => handleEditSectionClick(e, program._id, section)}
